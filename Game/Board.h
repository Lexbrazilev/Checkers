...
public:
    ...
    // Используется, если изменился размер окна.
...
private:
    ...
    // Функция для создания стартовой матрицы.
    void make_start_mtx()
    {
        for (POS_T i = 0; i < 8; ++i)
        {
    ...
    // Функция, которая перерисовывает все текстуры.
    void rerender()
    {
        // Рисуем доску
        SDL_RenderClear(ren);
        ...
        // Задержка для стабильности отображения на macOS.
        SDL_Delay(10);
