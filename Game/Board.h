#pragma once
#include <iostream>
#include <fstream>
#include <vector>

#include "../Models/Move.h"
#include "../Models/Project_path.h"

#ifdef __APPLE__
    #include <SDL2/SDL.h>
    #include <SDL2/SDL_image.h>
#else
    #include <SDL.h>
    #include <SDL_image.h>
#endif

using namespace std;

class Board
{
public:
    Board() = default;
    Board(const unsigned int W, const unsigned int H) : W(W), H(H)
    {
    }

    // Инициализация и отображение стартовой доски.
    int start_draw()
    {
        if (SDL_Init(SDL_INIT_EVERYTHING) != 0)
        {
            print_exception("SDL_Init can't init SDL2 lib");
            return 1;
        }
        if (W == 0 || H == 0) // Если не задан размер доски, то используем размер экрана.
        {
            SDL_DisplayMode dm;
            if (SDL_GetDesktopDisplayMode(0, &dm))
            {
                print_exception("SDL_GetDesktopDisplayMode can't get desctop display mode");
                return 1;
            }
            W = min(dm.w, dm.h);
            W -= W / 15;
            H = W;
        }
        win = SDL_CreateWindow("Checkers", 0, H / 30, W, H, SDL_WINDOW_RESIZABLE); // Создание окна для игры.
        if (win == nullptr)
        {
            print_exception("SDL_CreateWindow can't create window");
            return 1;
        }
        ren = SDL_CreateRenderer(win, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC); // Создание рендера для окна.
        if (ren == nullptr)
        {
            print_exception("SDL_CreateRenderer can't create renderer");
            return 1;
        }
        board = IMG_LoadTexture(ren, board_path.c_str()); // Загрузка игровых текстур.
        w_piece = IMG_LoadTexture(ren, piece_white_path.c_str());
        b_piece = IMG_LoadTexture(ren, piece_black_path.c_str());
        w_queen = IMG_LoadTexture(ren, queen_white_path.c_str());
        b_queen = IMG_LoadTexture(ren, queen_black_path.c_str());
        back = IMG_LoadTexture(ren, back_path.c_str());
        replay = IMG_LoadTexture(ren, replay_path.c_str());
        if (!board || !w_piece || !b_piece || !w_queen || !b_queen || !back || !replay) // Проверка успешности загрузки текстур.
        {
            print_exception("IMG_LoadTexture can't load main textures from " + textures_path);
            return 1;
        }
        SDL_GetRendererOutputSize(ren, &W, &H); // Подготовка доски
        make_start_mtx();
        rerender();
        return 0;
    }

    void redraw() // Сброс доски и перерисовка.
    {
        game_results = -1;
        history_mtx.clear();
        history_beat_series.clear();
        make_start_mtx();
        clear_active();
        clear_highlight();
    }

    void move_piece(move_pos turn, const int beat_series = 0) // Перемещение фигуры по доске но новую позицию.
    {
        if (turn.xb != -1)
        {
            mtx[turn.xb][turn.yb] = 0;
        }
        move_piece(turn.x, turn.y, turn.x2, turn.y2, beat_series); // Перемещаем фигуру
    }

    void move_piece(const POS_T i, const POS_T j, const POS_T i2, const POS_T j2, const int beat_series = 0)  // Ход фигуры
    {
        if (mtx[i2][j2]) // Проверка, что конечная позиция пуста
        {
            throw runtime_error("final position is not empty, can't move");
        }
        if (!mtx[i][j]) // Проверка, что начальная позиция не пуста
        {
            throw runtime_error("begin position is empty, can't move");
        }
        if ((mtx[i][j] == 1 && i2 == 0) || (mtx[i][j] == 2 && i2 == 7)) // Проверка на превращение в дамку
            mtx[i][j] += 2;
        mtx[i2][j2] = mtx[i][j];
        drop_piece(i, j);
        add_history(beat_series);
    }

    void drop_piece(const POS_T i, const POS_T j) // Удаление фигуры
    {
        mtx[i][j] = 0;
        rerender();
    }

    void turn_into_queen(const POS_T i, const POS_T j) // Превращение шашки в дамку
    {
        if (mtx[i][j] == 0 || mtx[i][j] > 2) // Проверка, что фигура может превратиться в дамку
        {
            throw runtime_error("can't turn into queen in this position");
        }
        mtx[i][j] += 2;
        rerender();
    }
    vector<vector<POS_T>> get_board() const  // Получение текущего состояния доски
    {
        return mtx;
    }

    void highlight_cells(vector<pair<POS_T, POS_T>> cells) // Выделение клетки на доске
    {
        for (auto pos : cells)
        {
            POS_T x = pos.first, y = pos.second;
            is_highlighted_[x][y] = 1;
        }
        rerender();
    }

    void clear_highlight() // Сброс выделения
    {
        for (POS_T i = 0; i < 8; ++i)
        {
            is_highlighted_[i].assign(8, 0);
        }
        rerender();
    }

    void set_active(const POS_T x, const POS_T y) // Установка активной клетки
    {
        active_x = x;
        active_y = y;
        rerender();
    }

    void clear_active() // Сброс активной клетки
    {
        active_x = -1;
        active_y = -1;
        rerender();
    }

    bool is_highlighted(const POS_T x, const POS_T y) // Выделена ли клетка
    {
        return is_highlighted_[x][y];
    }

    void rollback() // Отмена последнего хода
    {
        auto beat_series = max(1, *(history_beat_series.rbegin())); // Получаем количество сбитых фигур
        while (beat_series-- && history_mtx.size() > 1)
        {
            history_mtx.pop_back();
            history_beat_series.pop_back();
        }
        mtx = *(history_mtx.rbegin()); // Восстанавливаем состояние доски
        clear_highlight();
        clear_active();
    }

    void show_final(const int res) // Отображение финального результата игры
    {
        game_results = res;
        rerender();
    }

    // use if window size changed
    void reset_window_size() // Используется, если изменился размер окна
    {
        SDL_GetRendererOutputSize(ren, &W, &H);
        rerender();
    }

    void quit() // Завершение работы и освобождение ресурсов
    {
        SDL_DestroyTexture(board);
        SDL_DestroyTexture(w_piece);
        SDL_DestroyTexture(b_piece);
        SDL_DestroyTexture(w_queen);
        SDL_DestroyTexture(b_queen);
        SDL_DestroyTexture(back);
        SDL_DestroyTexture(replay);
        SDL_DestroyRenderer(ren);
        SDL_DestroyWindow(win);
        SDL_Quit();
    }

    ~Board() // Деструктор
    {
        if (win)
            quit();
    }

private:
    void add_history(const int beat_series = 0) // Добавление текущего состояния доски в историю
    {
        history_mtx.push_back(mtx);
        history_beat_series.push_back(beat_series);
    }
    // function to make start matrix
    void make_start_mtx() // Функция для создания стартовой матрицы
    {
        for (POS_T i = 0; i < 8; ++i)
        {
            for (POS_T j = 0; j < 8; ++j)
            {
                mtx[i][j] = 0;
                if (i < 3 && (i + j) % 2 == 1)
                    mtx[i][j] = 2;
                if (i > 4 && (i + j) % 2 == 1)
                    mtx[i][j] = 1;
            }
        }
        add_history();
    }

    // function that re-draw all the textures
    void rerender() // Функция, которая перерисовывает все текстуры
    {
        // draw board
        SDL_RenderClear(ren);
        SDL_RenderCopy(ren, board, NULL, NULL); // Рисуем доску

        // draw pieces
        for (POS_T i = 0; i < 8; ++i) // Рисуем фигуры
        {
            for (POS_T j = 0; j < 8; ++j)
            {
                if (!mtx[i][j])
                    continue; // Пропускаем пустые клетки
                int wpos = W * (j + 1) / 10 + W / 120;
                int hpos = H * (i + 1) / 10 + H / 120;
                SDL_Rect rect{ wpos, hpos, W / 12, H / 12 };

                SDL_Texture* piece_texture; // Указатель на текстуру фигуры
                if (mtx[i][j] == 1) // Определяем текстуру фигуры по её типу
                    piece_texture = w_piece; // Белые
                else if (mtx[i][j] == 2)
                    piece_texture = b_piece; // Черные
                else if (mtx[i][j] == 3)
                    piece_texture = w_queen; // Белая дамка
                else
                    piece_texture = b_queen; // Черная дамка

                SDL_RenderCopy(ren, piece_texture, NULL, &rect); // Рисуем фигуры на доске
            }
        }

        // draw hilight
        SDL_SetRenderDrawColor(ren, 0, 255, 0, 0); // Рисуем клетки
        const double scale = 2.5;
        SDL_RenderSetScale(ren, scale, scale); // Устанавливаем масштаб
        for (POS_T i = 0; i < 8; ++i)
        {
            for (POS_T j = 0; j < 8; ++j)
            {
                if (!is_highlighted_[i][j])
                    continue;
                SDL_Rect cell{ int(W * (j + 1) / 10 / scale), int(H * (i + 1) / 10 / scale), int(W / 10 / scale),
                              int(H / 10 / scale) };
                SDL_RenderDrawRect(ren, &cell);
            }
        }

        // draw active
        if (active_x != -1) // Рисуем активную клетку
        {
            SDL_SetRenderDrawColor(ren, 255, 0, 0, 0); // Устанавливаем цвет для активной клетки
            SDL_Rect active_cell{ int(W * (active_y + 1) / 10 / scale), int(H * (active_x + 1) / 10 / scale),
                                 int(W / 10 / scale), int(H / 10 / scale) }; // Создаем прямоугольник для активной клетки
            SDL_RenderDrawRect(ren, &active_cell); // Рисуем рамку вокруг активной клетки
        }
        SDL_RenderSetScale(ren, 1, 1);

        // draw arrows
        SDL_Rect rect_left{ W / 40, H / 40, W / 15, H / 15 }; // Рисуем стрелки для действий
        SDL_RenderCopy(ren, back, NULL, &rect_left);
        SDL_Rect replay_rect{ W * 109 / 120, H / 40, W / 15, H / 15 };
        SDL_RenderCopy(ren, replay, NULL, &replay_rect);

        // draw result
        if (game_results != -1) // Рисуем результат игры
        {
            string result_path = draw_path; // Путь к изображению результата
            if (game_results == 1)
                result_path = white_path; // Путь к изображению победы белых
            else if (game_results == 2)
                result_path = black_path; // Путь к изображению победы черных
            SDL_Texture* result_texture = IMG_LoadTexture(ren, result_path.c_str()); // Загружаем текстуру результата
            if (result_texture == nullptr)
            {
                print_exception("IMG_LoadTexture can't load game result picture from " + result_path);
                return;
            }
            SDL_Rect res_rect{ W / 5, H * 3 / 10, W * 3 / 5, H * 2 / 5 };
            SDL_RenderCopy(ren, result_texture, NULL, &res_rect); // Рисуем результат
            SDL_DestroyTexture(result_texture);
        }

        SDL_RenderPresent(ren); // Обновляем экран
        // next rows for mac os
        SDL_Delay(10); // Задержка для стабильности отображения на macOS
        SDL_Event windowEvent;
        SDL_PollEvent(&windowEvent);
    }

    void print_exception(const string& text) { // Функция для записи ошибок в лог-файл
        ofstream fout(project_path + "log.txt", ios_base::app);
        fout << "Error: " << text << ". "<< SDL_GetError() << endl;
        fout.close();
    }

  public:
    int W = 0;
    int H = 0;
    // history of boards
    vector<vector<vector<POS_T>>> history_mtx; // История состояний доски

  private:
    SDL_Window *win = nullptr;
    SDL_Renderer *ren = nullptr;
    // Текстуры для игры
    SDL_Texture *board = nullptr; 
    SDL_Texture *w_piece = nullptr;
    SDL_Texture *b_piece = nullptr;
    SDL_Texture *w_queen = nullptr;
    SDL_Texture *b_queen = nullptr;
    SDL_Texture *back = nullptr;
    SDL_Texture *replay = nullptr;
    // Путь к файлам текстур
    const string textures_path = project_path + "Textures/";
    const string board_path = textures_path + "board.png";
    const string piece_white_path = textures_path + "piece_white.png";
    const string piece_black_path = textures_path + "piece_black.png";
    const string queen_white_path = textures_path + "queen_white.png";
    const string queen_black_path = textures_path + "queen_black.png";
    const string white_path = textures_path + "white_wins.png";
    const string black_path = textures_path + "black_wins.png";
    const string draw_path = textures_path + "draw.png";
    const string back_path = textures_path + "back.png";
    const string replay_path = textures_path + "replay.png";
    // Координаты выбранной ячейки
    int active_x = -1, active_y = -1;
    // Результат игры, если он существует
    int game_results = -1;
    // Матрица возможных ходов
    vector<vector<bool>> is_highlighted_ = vector<vector<bool>>(8, vector<bool>(8, 0));
    // Матрица возможных ходов
    // 1 - белая фигура, 2 - черная фигура, 3 - белая ферзь, 4 - черная ферзь
    vector<vector<POS_T>> mtx = vector<vector<POS_T>>(8, vector<POS_T>(8, 0));
    // Серия побитий для каждого хода
    vector<int> history_beat_series;
};
