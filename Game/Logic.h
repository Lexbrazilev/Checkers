...
class Logic
{
  public:
    ...
    vector<move_pos> find_best_turns(const bool color)
    {
        ...
        do {
            res.push_bak(next_move[state]); // Добавляем ходы в результат.
...
private:
                                ...
                                double alpha = -1)
    {
        ...
        auto now_thave_beats = have_beats; // Создаем копию have_beats что бы сохранить значения.

        if (!now_thave_beats && state != 0) // Если не кого бить и нет уже ходов то ищем заново.
        { 
        ...
        for (auto turn : now_turns) { // Перебираем все ходы.
            ...
            double score;
            if (now_thave_beats) // Если есть кого бить, то продолжаем серию.
            { 
